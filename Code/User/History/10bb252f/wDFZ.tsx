...
const Autocomplete = (props) => {
  ...
  const onChange = e => {
    ...
    suggestions.map((school) => (
      <div onClick={() => actionFunction(school)}>{school.name}</div>))}
    ...
    actionFunction(e.currentTarget.value);
  };
const onClick = e => {
    setActive(0);
    setFiltered([]);
        ...
        return (
          <ul className="autocomplete  border border-solid border-gray-400 border-t-0 list-none my-0 pl-0 max-h-36 overflow-y-auto w-80">
            {filtered.map((suggestion, index) => {
              ...
              return (
                <li className="${className} p-2 hover:bg-gray-600 hover:cursor-pointer hover:font-bold" key={`${suggestion}-${index}`} onClick={onClick}>
                  {suggestion}
                </li>
              );
            })}
